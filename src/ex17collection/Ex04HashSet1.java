...
-하지만 개발자가 정의한 클래스라면 hashCode(), equals()를 
	오버라이딩해야 한다.
-List가 배열의 특성을 가진다면, set은 집합의특성을 가진다.
*/
...
public class Ex04HashSet1 {
	...
	public static void main(String[] args) {
		...
		String vaString2 = new String("개발자");
		Date varDate = new Date();
		int varInt = 100;//저장시 Integer로 Boxing처리됨
		Teacher vaTeacher = new Teacher("김봉두", 55, "체육");
		...
		System.out.println(set.add(vaString2) ? "저장성공":"저장실패");
		//저장에 실패했으므로 5개 출력됨
		...
		인스턴스 삭제 : set은 인덱스를 사용하지 않으므로 인스턴스의
			참조값을 통해 삭제된다. */
		System.out.println(set.remove(vaString2) ?
