...
있다.
2.자식에서 오버라딩한 메서드가 우선적으로 호출된다.
3.자식에서 새롭게 정의한 멤버는 접근할 수 없다.
4.이것을 이질화(Hererogeneous:헤테로지니어스)라고 표현한다.

-동일한 타입의 참조변수로 인스턴스를 참조
1.인스턴스 전체를 접근할 수 있다. 즉 클래스의 일반적인 규칙을 따른다.
2.이것을 통질화(Homogeneous:호모지니어스)라고 표현한다.
 */
//부모클래스
class MyParent {
	int parentMember;
...
class MyChild extends MyParent{
	...
	/*
	메서드명은 동일하지만 매겨변수가 틀리므로 어버로딩을 정의한
	메서드.따라서 지식에서 확장된
	 */
...
public class E06Polymorphism {
	...
	public static void main(String[] args) {
		...
		//동질화: 참조변수와 인스턴스의 타임이 동일
		MyChild myChild = new MyChild();
		//동질화의 경우 인스탄스 전체를 접근헐 수 있다.
		...
		/*
		이질화 : 부모타입의 변수로 자식 인스턴슬르 참조한느것을 말한다.
		기영우 자식 클래스에서 새롭게 정의한 멤버는 접근살 수 없다.
		접근하고 싷다면 자식타입으로 강제형변환(다운캐스팅)해야한다.
		...
		//부모 참수변수로 자식 인스턴스를 참조한 두번째 인스턴스 생성
		...
		하지만 아래와같이 기술하면 에러가 발생한다.
		 */
		myParent.parentMember=1;
		/*소괄호를 하나 더 추가해서 myParent를 자식 타입으로 형변환 후
		멤버 변수에 접근해야 한다. */
		...
		/*자식타입으로 형변화 이후에 해당 참조변수를 통해 자식 멤버에 
		접근한다. 위와 동일한 방식이다. */
