...
public class E03ReferenceYesNo {
	...
	public static void main(String[] args) {
		...
		/*
		부모 참조변수로 자식 인스턴스를 참조할 수 있다. 좌측은부모 
		우측은 좌측이므로 모두 실행가능한 문장이다. */
		MyA refNum1 = new MyB();
		MyA refNum2 = new MyC();
		MyB refNum3 = new MyC();
		...
		//MyA -> MyC를 참조하므로 가능
		MyA refId1 = new MyC();
		//MyB ->MyA -> MyA를 참조하므로 불가능(자식은 부모를 참조할 수 없다.)
		//MyB refId2 = refId;
		//MyC -> MyA를 참조하므로 불가능
		//MyC refId3 = refID;
		/*
		refId1은 MyC인스턴스를 참조하지만, 원래 MyA타입이므로
		MyC타입이라 착각하면 안된다.  */
		//MyB refId2 = refId1;
		//MyC refId3 = refId1;
		...
		있기 때문에 가능하고, 방식은 기본자료형과 동일하게 소괄호를 
		이용하면된다. */
