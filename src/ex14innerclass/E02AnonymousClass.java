...
인터페이스 정의 : 기능이 없는 추상메서드만 멤버로 정의할 수 있다.
	또한 메서드 앞에는 public abstract가 생략됟어 있다. */
interface ISinger {
	public void dancing();
	}
public class E02AnonymousClass {
	...
	public static void main(String[] args) {
		...
		//자식쪽의 toString이 호춣되어 팀명까지 출력된다.
		System.out.println(s2);
		
		/*
		GirlGroup 클래스를 상속받은 익명클래스 정의
		: Singer타입의 변수에 GirlGroup을 상속받은 익명 클래스를
		생성해서 그 주소값을 할당한다.
		개념적으로 newGirlGroup(){};
		...
		Singer s3 = new GirlGroup("아이린", "레드벨벳") {
			/* 자식에서 확장한 멤버변수. 해당변수는 익명클래스 내부
			에서만 사용할 수 있다. */
			@Override
			public String toString() {
				return super.toString()+", 소속하:SM";
			}
		};
		/*
		...
		주된 목적이므로, 부모클래스는 추상메서드만 정의되어 있어도
		아무 상관이 없다. */
