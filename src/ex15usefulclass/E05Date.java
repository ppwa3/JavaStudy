...
public class E05Date {
	...
	/*
	Calender 클래스를 통해 얻은 인스턴스를 초(Sec)로 변환한 후 
	매개변수로 전달된 날짜 서식으로 변환한 문자열을 반환한다. */
	...
	public static void main(String[] args) {
		/*
		Calender 클래스는 Java의 대표적인 싱글톤 객체이므로
		...
		사용한다. 두번째 인수가 음수면 과거의 날짜를 구할수있다. */
	...
	public static long myDiffDate(String today, String future) {
		...
		try {
			/*
			매게변수를 통해 전달된 2개의 문자열은 날짜의 서식이 서로 다르다.
			따라서 각 날짜의 서식을 명확히 지정해야만 날짜를 인식하여 연산
			할 수 있다. */
			//각 서식의 날짜를 파싱해서 Date 인스턴스로 변환한다.
			Date formatToday = new SimpleDateFormat
					("yyyy-MM-dd").parse(today);
			Date formatFuture = new SimpleDateFormat
					("yyyy/MM/dd").parse(future);
